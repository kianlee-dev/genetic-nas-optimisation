...
ga = gnas.genetic_algorithm_searcher(ss, generation_size=config.get('generation_size'),
                                     ...
                                     p_cross_over=config.get('p_cross_over'), gaussian_s =config.get('gaussian_s'),
                                     cross_over_type=config.get('cross_over_type'), epoch= config.get('n_epochs'),
...
if model_type == ModelType.CNN:
    ...
    for epoch in range(config.get('n_epochs')):  # loop over the dataset multiple times
        ...
        formatted_elapsed_time = time.strftime("%H:%M:%S", time.gmtime(total_elapsed_time))
        n_diff_list.append(n_diff)
        print(
            '|Epoch: {:2d}|Time: {:2.3f}|Loss:{:2.3f}|Accuracy: {:2.3f}%|Validation Accuracy: {:2.3f}%|LR: {:2.3f}|N Change : {:2d}|Elapsed Time: {}'.format(
                epoch, (
                               time.time() - s) / 60,
                       running_loss / i,
...
print('List of Updates to Population for Each Epoch:', n_diff_list)