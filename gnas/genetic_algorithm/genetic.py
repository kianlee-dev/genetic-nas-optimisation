...
from gnas.search_space.cross_over import individual_uniform_crossover, individual_block_crossover, individual_blend_crossover
from gnas.search_space.mutation import individual_combined_mutation, individual_adaptive_flip_mutation, individual_flip_mutation
...
def genetic_algorithm_searcher(search_space: SearchSpace, generation_size=20, population_size=300, keep_size=0,
                               min_objective=True, mutation_p=None, p_cross_over=None, cross_over_type='Bit', gaussian_s=None, epoch = None):
    if mutation_p is None: mutation_p = 1 / search_space.n_elements
    if gaussian_s is None: gaussian_s = 1 / search_space.n_elements
    if p_cross_over is None: p_cross_over = 1
    print(f'p mutation: {mutation_p:.5f}, gaussian s: {gaussian_s:.5f}, 1/n_elements: {1/search_space.n_elements:.5f}')

    def population_initializer(p_size):
        return search_space.generate_population(p_size)

    def mutation_function(x, p):
        return individual_flip_mutation(x, p)
    ...
    elif cross_over_type == 'Blend':
        print("Blend base cross over")

        def cross_over_function(x0, x1):
            return individual_blend_crossover(x0, x1, p_cross_over)
    else:
        raise Exception('')
    ...
    return GeneticAlgorithms(population_initializer, mutation_function, cross_over_function, selection_function,
                             ...
                             population_size=population_size, keep_size=keep_size, epoch=epoch, mutation_p = mutation_p)
...
class GeneticAlgorithms(object):
    def __init__(self, population_initializer, mutation_function, cross_over_function, selection_function,
                 population_size=300, generation_size=20, keep_size=20, min_objective=False, mutation_p=None, epoch=None):
        ####################################################################
        # Functions
        ####################################################################
    ...
    def _create_new_generation(self, population, population_fitness):
        ...
        new_generation = np.asarray([self.mutation_function(c, self.mutation_p) for c in child])  # mutation
    ...
    def update_population(self):
        ...
        if (self.epoch_tracker /2 == 0.2 * self.epoch):
            self.set_mutation_probability()
        if (self.epoch_tracker /2 == 0.4 * self.epoch):
            self.set_mutation_probability()
        if (self.epoch_tracker /2 == 0.6 * self.epoch):
            self.set_mutation_probability()
        if (self.epoch_tracker /2 == 0.8 * self.epoch):
            self.set_mutation_probability()
        if (self.epoch_tracker /2 == 0.9 * self.epoch):
            self.set_mutation_probability()
        ...
        if (self.counter == 4):
            self.counter = 0
            if (self.mutation_p < 1.3 * self.default_p):
             #   print("mutation_p updated")
                self.mutation_p = self.mutation_p * 1.07
    ...
    def sample_child(self):
        if len(list(self.max_dict.keys())) == 0: # if not population exist generate random indivaul
            ...
            return self.mutation_function(child[0], self.mutation_p) # select the first then mutation
    def set_mutation_probability(self):
        self.mutation_p = self.default_p
